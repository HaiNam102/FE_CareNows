...
function App() {
  const handleBlockUser = (user) => {
    // Gọi API hoặc cập nhật state để chặn tài khoản
    alert(`Chặn tài khoản: ${user.name}`);
  ...
  const handleActivateUser = (user) => {
    // Gọi API hoặc cập nhật state để kích hoạt tài khoản
    alert(`Kích hoạt tài khoản: ${user.name}`);
    setUsers(prev =>
      prev.map(u =>
