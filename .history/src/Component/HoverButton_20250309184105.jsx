...
const HoverButton = ({ text, showArrow = true, size = "medium" }) => {
  ...
  const { padding, fontSize, height, minHeight: height, arrowSize, arrowFontSize } = sizes[size] || sizes.medium;

  return (
    <div
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      style={{ boxSizing: "border-box", display: "flex", alignItems: "center", gap: hovered && showArrow ? "10px" : "2px", transition: "gap 0.4s ease-in-out" }}
    >
      <button
        style={{
          position: "relative",
