...
const SpecialNotesInput = ({ value, onChange, placeholder }) => {
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      {/* Dropdown gợi ý */}
      {showSuggestions && (
        <div className="absolute z-10 mt-1 w-full max-h-96 overflow-y-auto bg-white border border-gray-200 rounded-md shadow-lg">
          ...
          <div className="divide-y divide-gray-100">
            {filteredCategories.map((category, categoryIndex) => (
              <div key={categoryIndex}>
                <div className="p-2 bg-gray-50 text-sm font-medium sticky top-14 z-10">
                  {category.category}
                </div>
                {category.notes.map((note, noteIndex) => (
                  <div
                    key={`${categoryIndex}-${noteIndex}`}
