...
const HoverButton = ({ text, showArrow = true, size = "medium", variant = "outline", onClick, className }) => {
  const [hovered, setHovered] = useState(false);
  const { fontSize, height, arrowFontSize } = sizes[size] || sizes.medium;
  const { borderColor, backgroundColor, textColor, hoverTextColor } = variants[variant] || variants.outline;

  const isOutline = variant === "outline";
  const currentTextColor = isOutline ? (hovered ? hoverTextColor : textColor) : (hovered ? hoverTextColor : textColor);
      ...
      >
        ...
        <span
          style={{
            ...
            transform: isOutline 
              ? (hovered ? "translateY(0)" : "translateY(100%)")  // Đảo ngược animation cho outline
