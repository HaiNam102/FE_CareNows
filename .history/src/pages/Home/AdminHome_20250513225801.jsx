    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f7f9fa', fontFamily: 'SVN-Gilroy' }}>
      ...
      <div style={{ flex: 1, padding: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 32, marginTop: 32 }}>
          ...
          {/* Quản lý chuyên viên */}
          <div style={{ background: '#fff', borderRadius: 24, boxShadow: '0 2px 8px #0001', padding: 24, flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            ...
            <ResponsiveContainer width={300} height={300} style={{ alignSelf: 'center' }}>
              <PieChart>
                <defs>
                  <linearGradient id={pieGradients.green.id} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={pieGradients.green.startColor} />
                    <stop offset="100%" stopColor={pieGradients.green.endColor} />
                  </linearGradient>
                  <linearGradient id={pieGradients.pink.id} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={pieGradients.pink.startColor} />
                    <stop offset="100%" stopColor={pieGradients.pink.endColor} />
                  </linearGradient>
                  <linearGradient id={pieGradients.gray.id} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={pieGradients.gray.startColor} />
                    <stop offset="100%" stopColor={pieGradients.gray.endColor} />
                  </linearGradient>
                  <linearGradient id={pieGradients.grayStroke.id} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={pieGradients.grayStroke.startColor} />
                    <stop offset="100%" stopColor={pieGradients.grayStroke.endColor} />
                  </linearGradient>
                </defs>
                <Pie 
                  data={pieData} 
                ...
                >
                  {pieData.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={`url(#${index === 0 ? pieGradients.green.id : pieGradients.pink.id})`}
                      stroke="none"
                    />
                  ))}
                </Pie>
                {/* Outer dash stroke circle */}
                <Pie 
                  data={[{ name: "OuterStroke", value: 100 }]} 
                  dataKey="value"
                  innerRadius={100}
                  outerRadius={102}
                  fill="none"
                ...
                <Pie 
                  ...
                  startAngle={0}
                  endAngle={360}
                  fill={`url(#${pieGradients.gray.id})`}
                  stroke="#CCCCCC"
                  strokeWidth={1}
                />
              </PieChart>
            </ResponsiveContainer>
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center', width: 140, pointerEvents: 'none', zIndex: 10 }}>
