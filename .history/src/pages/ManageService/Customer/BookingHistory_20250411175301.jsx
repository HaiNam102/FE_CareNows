  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/booking/customer`, {
          headers: {
        ...
        if (response.status === 200 && response.data?.data) {
          const bookingsWithDetails = response.data.data.map(booking => ({
            ...booking,
            careTaker: {
              name_of_care_taker: booking.careTaker?.name_of_care_taker || 'Chưa có thông tin',
              email: booking.careTaker?.email,
              phone_number: booking.careTaker?.phone_number,
              introduce_yourself: booking.careTaker?.introduce_yourself,
              gender: booking.careTaker?.gender,
              district: booking.careTaker?.district,
              ward: booking.careTaker?.ward,
              workable_area: booking.careTaker?.workable_area,
              experience_year: booking.careTaker?.experience_year,
              service_price: booking.careTaker?.service_price,
              average_rating: booking.careTaker?.average_rating || 0,
              avatar: booking.careTaker?.avatar
            },
            careRecipient: {
              name: booking.careRecipient?.name || 'Chưa có thông tin',
              gender: booking.careRecipient?.gender,
              year_old: booking.careRecipient?.year_old,
              special_detail: booking.careRecipient?.special_detail
            },
                        ...
                        <div className="flex items-start gap-6 pt-8">
                          ...
                          <div className="flex flex-col items-start gap-3 flex-1">
                            ...
                            <div className="self-stretch [font-family:'SVN-Gilroy-Medium',Helvetica] font-medium text-[#8c8c8c] text-[15px] leading-[26.3px]">
                              ...
                              <div className="mb-2">
                                <strong>Người được chăm sóc:</strong> {booking.careRecipient?.name || 'Chưa có thông tin'}
                              </div>
                            </div>
                          </div>
                        </div>
