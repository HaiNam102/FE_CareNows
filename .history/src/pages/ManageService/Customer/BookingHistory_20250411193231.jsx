...
import { bookingApi, storeMockToken } from '../../../services/api';
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg w-full max-w-3xl p-6">
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
            <p className="text-red-600">{booking.errorMessage || 'Could not load complete booking details'}</p>
            <p className="text-gray-600 text-sm mt-2">Showing partial information that was available.</p>
          </div>
        </div>
      </div>
    );
  ...
  const handleViewDetail = async (booking) => {
    try {
      ...
      if (response.data && response.data.data) {
        ...
        console.error('Invalid API response format for booking details', response);
      ...
      if (error.response && error.response.status === 404) {
        ...
        setSelectedBooking({...booking, loadError: true, errorMessage: 'Failed to load complete details'});
      }
    }
  };
