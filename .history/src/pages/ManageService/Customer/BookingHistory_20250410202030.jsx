            ...
            <div className="grid grid-cols-2 gap-6 mb-[400px]">
              {sortedBookings.map((booking) => {
                ...
                return (
                  <div
                    key={booking.id}
                    className={`relative overflow-hidden transition-all duration-300 ease-in-out ${isActive ? "z-10" : "z-0"}`}
                  >
                    <Card className={`w-full bg-white rounded-lg transition-all duration-300 ${
                      isActive ? "shadow-lg hover:shadow-xl hover:-translate-y-1" : "shadow-sm opacity-85"
                    }`}>
                      {activeCategory !== "all" && !isActive && (
                        <div className="absolute inset-0 z-10 bg-gray-100/30" />
                      )}
