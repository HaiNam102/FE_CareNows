...
const ProfileCustomer = () => {
  ...
  const handleSave = async (field) => {
    try {
      ...
      // First, get the customer ID from the token
      const response = await axios.get('http://localhost:8080/api/customer/getByCustomerId', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const updateResponse = await axios.put('http://localhost:8080/api/customer', updateData, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (updateResponse.data.code === 200) {
        ...
        throw new Error(updateResponse.data.message || 'Cập nhật thất bại');
      }
    } catch (error) {
