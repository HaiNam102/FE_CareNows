...
const EditCareRecipientModal = ({ isOpen, onClose, onSuccess, recipient }) => {
  ...
  const fetchRecipientDetails = async (recipientId) => {
    try {
      const response = await axios.get(`${API_URL}/careRecipient/${recipientId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      if (response.data && response.data.data) {
        ...
        setFormData({
          ...
          gender: recipientData.gender || 'MALE',
          phoneNumber: recipientData.phoneNumber || '',
          dateOfBirth: recipientData.dateOfBirth ? recipientData.dateOfBirth.split('T')[0] : '',
          specialDetail: recipientData.specialDetail || ''
        });
      }
    } catch (err) {
  ...
  // Fetch recipient details when modal opens
  useEffect(() => {
    if (isOpen && recipient?.careRecipientId) {
      fetchRecipientDetails(recipient.careRecipientId);
    }
  }, [isOpen, recipient]);
