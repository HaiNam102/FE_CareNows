...
const ProfileCustomer = () => {
  ...
  const handleSave = async (field) => {
    try {
      ...
      // Prepare the update data according to CustomerReq DTO
      const updateData = {
        nameOfCustomer: field === 'name' ? editValues.name : userData.name,
      ...
      if (response.data.code === 200) {
        ...
        setUserData(prev => ({
          ...
          ...(field === 'address' ? {
            ward: editValues.address.split(', ')[0],
            district: editValues.address.split(', ')[1]
          } : {}),
          ...(field === 'phone' ? { phone: editValues.phone } : {}),
          ...(field === 'name' ? { name: editValues.name } : {})
        }));

        setEditMode(prev => ({ ...prev, [field]: false }));
      } else {
        throw new Error(response.data.message || 'Failed to update customer data');
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        setError(error.response.data.message || 'Failed to update customer data');
