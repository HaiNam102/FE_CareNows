            ...
            <div className="grid grid-cols-1 gap-4 mb-[400px]">
              {filteredBookings.map((booking) => {
                ...
                return (
                  ...
                  >
                    <div className="border text-card-foreground shadow bg-white rounded-lg">
                      <div className="flex flex-col space-y-1.5 p-6 px-5 py-1">
                        <div className="flex items-center justify-between">
                          <div className="[font-family:'SVN-Gilroy-Bold',Helvetica] font-bold text-black text-base leading-[26.3px]">
                            Ngày book đơn: {new Date(booking.created_at).toLocaleDateString('vi-VN')}
                          </div>
                          <div className={`inline-flex items-center border transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent shadow hover:bg-primary/80 ${statusConfig.color} text-white font-medium text-[13px] px-2.5 py-0 rounded`}>
                            {statusConfig.text}
                          </div>
                        </div>
                        <div className="[font-family:'SVN-Gilroy-Medium',Helvetica] font-medium text-[#8c8c8c] text-[13px] leading-[26.3px]">
                          Đơn hàng: #{booking.booking_id}
                        </div>
                        <div data-orientation="horizontal" role="none" className="shrink-0 bg-border h-[1px] w-full"></div>
                      </div>

                      <div className="p-6 pt-0 px-5 pb-0">
                        <div className="flex items-start gap-3">
                          <div 
                            className="w-[130px] h-[130px] bg-cover bg-center rounded-full"
                            style={{ backgroundImage: `url(${booking.careTaker?.avatar || '/ellipse-18-3.png'})` }}
                          />
                          <div className="flex flex-col items-end gap-1 flex-1">
                            ...
                            <div className="flex items-center self-stretch">
                              <Star className="w-[15.31px] h-[15.31px] text-[#00a37d]" />
                              <div className="[font-family:'SVN-Gilroy-Bold',Helvetica] font-bold text-[14.2px] leading-[17.1px]">
                                <span className="text-[#00a37d]">{booking.careTaker?.average_rating || 5}</span>
                                <span className="text-[#111111]">&nbsp;</span>
                                <span className="text-[#bcb9c5]">({booking.careTaker?.total_reviews || 20})</span>
                              </div>
                            </div>
                            ...
                            {booking.service_progress === 'COMPLETED' && (
                              <div className="self-stretch [font-family:'SVN-Gilroy-Medium',Helvetica] font-medium text-[#008666] text-[13px] leading-[26.3px] underline">
                                Đánh giá ngay
                              </div>
                            )}
                            <div className="font-heading-h5-h5-semibold text-[#00a37d]">
                              {(booking.careTaker?.service_price || 500000).toLocaleString('vi-VN')} VNĐ
                            </div>
                          </div>
                        </div>
                      </div>

                      <div className="p-6 flex flex-col items-center gap-[5.84px] px-5 pt-0">
                        <div data-orientation="horizontal" role="none" className="shrink-0 bg-border h-[1px] w-full"></div>
                        <div className="flex items-center justify-between w-full">
                          <div className="[font-family:'SVN-Gilroy-Medium',Helvetica] font-medium text-primaryblue-800 text-[13px] leading-[26.3px]">
                            Xem chi tiết
                          </div>
