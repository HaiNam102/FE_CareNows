  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        if (response.data?.code === SuccessCode.GET_SUCCESSFUL.code && response.data?.data) {
          const bookingsData = response.data.data;
          const bookingsWithDetails = bookingsData.map(booking => ({
            booking_id: booking.id,
            created_at: booking.createdAt,
            service_progress: booking.status,
            ...
            careTaker: {
              name_of_care_taker: booking.careTaker?.fullName || 'Chưa có thông tin',
              email: booking.careTaker?.email,
              phone_number: booking.careTaker?.phoneNumber,
              ...
              service_price: booking.careTaker?.servicePrice,
              average_rating: booking.careTaker?.rating || 0,
              avatar: booking.careTaker?.avatar
            },
            careRecipient: {
              name: booking.careRecipient?.fullName || 'Chưa có thông tin',
              gender: booking.careRecipient?.gender,
              year_old: booking.careRecipient?.age,
              special_detail: booking.careRecipient?.specialNote
            },
            bookingDays: booking.bookingDays?.map(day => ({
              day: day.date,
              booking_id: day.bookingId
            })) || []
          }));
