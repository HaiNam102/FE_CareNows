...
const SignUpClient = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'district') {
      ...
      setAvailableWards(DANANG_WARDS[value] || []);
    }
  ...
  const checkUsernameExists = async (username) => {
    try {
      ...
      return response.data.some(account => account.user_name === username);
    } catch (error) {
  ...
  const createCustomer = async (customerData) => {
    ...
    const maxCustomerId = response.data.length > 0 
      ? Math.max(...response.data.map(c => Number(c.customer_id)))
      : 0;
  ...
  // Style cho dropdown không thay đổi
  ...
  useEffect(() => {
    const loadDistricts = async () => {
      try {
        ...
        console.error('Không thể tải danh sách quận:', error);
