...
const SignUpClient = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'district') {
      ...
      const formattedWards = wardsList.map(ward => ({
        value: ward,
        label: ward
      }));
      setAvailableWards(formattedWards);
    }
  ...
  useEffect(() => {
    // Chuyển đổi mảng districts thành format phù hợp cho dropdown
    const formattedDistricts = DANANG_DISTRICTS.map(district => ({
      value: district.name,
      label: district.name
    }));
